...
public class RestApi implements ApiApi, DefaultApi, ConformanceApi {
    ...
    public ResponseEntity<?> getCollections(
            @Parameter(in = ParameterIn.QUERY, description = "Property to be return" ,schema=@Schema())
                @Valid @RequestParam(value = "property", required = false) List<String> property,
        ...
        if (CQLFilterType.convert(filterLang) == CQLFilterType.CQL && CQLCrsType.convertFromUrl(crs) == CQLCrsType.EPSG4326) {
            ...
            return commonService.getCollectionList(q, filter, OGCMediaTypeMapper.json, CQLCrsType.convertFromUrl(crs), stacToCollection::convert, property);
        }
        else {
